...
La primera pasada del diagnóstico de páginas de categoría del 2026-09-20, bajo el mismo contrato `8 / 16 / 28` + JSF `8 / 2`, produjo `47.61s` con 1 worker, `46.03s` con 2, `48.17s` con 3 y `49.09s` con 4. Las cuatro corridas mantuvieron `523/523` y `0` errores. La mejora aislada de 2 workers frente a 1 fue pequeña (`1.58s`) y no es suficiente para establecer reproducibilidad; 3 y 4 fueron más lentos. Se conserva `1` como default productivo y se requieren controles repetidos antes de cerrar el diagnóstico. Quality CI runs `2093`–`2098` finalizaron en `success`, incluyendo la prueba de concurrencia.
...
- [ ] Cerrar la comparación de `FCM_BENCH_CATEGORY_PAGE_WORKERS` bajo `8 / 16 / 28` + JSF `8 / 2` mediante controles repetidos, con cobertura completa y `0` errores como requisito.
