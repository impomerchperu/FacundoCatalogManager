...
def _facundo_direct_pages(scraper: CategoryScraper, category_url: str, first_html: str, expected_count: int) -> tuple[list[str], int]:
...
def _remember_jsf_settings(category_id: int, category_html: str) -> None:
    ...
    with _JSF_STATE_LOCK:
        _JSF_REQUEST_STATE[category_id] = {"query": dict(query), "settings": dict(request_settings)}
    ...
    if isinstance(query_props, dict):
        _remember_jsf_metadata(category_id, CategoryScraper._to_int(query_props.get("found_posts")), CategoryScraper._to_int(query_props.get("max_num_pages")))
...
def _apply_live_query_defaults(values: dict[str, str], query: object) -> None:
    ...
    for key in ("posts_per_page", "no_results_text", "disable_query_merge", "is_archive_main_query", "post_status"):
        value = query.get(key)
        if value is None:
            continue
...
def _browser_compatible_jsf_payload(category_id: int, page: int) -> list[tuple[str, str]]:
    ...
    return [(key, values.get(key, value)) for key, value in payload if key != "indexing_filters[]"]
...
def _probe_boundary_page(self: CategoryScraper, category_url: str, category_id: int, page: int, seen_product_keys: set[str]) -> tuple[bool, set[str]]:
...
def _jsf_category_pages_with_probe(self: CategoryScraper, category_url: str, category_id: int, expected_count: int, category_html: str = "") -> list[str]:
    """Walk known JSF pages and validate the boundary without over-probing."""
    _remember_jsf_settings(category_id, category_html)
    first_fetcher = self._fetch_jsf_page
    if getattr(first_fetcher, "__func__", None) is not _retry_jsf_page:
        first_fetcher = _retry_jsf_page
    found_posts, declared_max, first_html = first_fetcher(self, category_url, category_id, 1)
    if not first_html:
        return [category_url]
    expected_pages = self._required_page_count(expected_count)
    published_pages = self._required_page_count(found_posts)
    response_html_pages = max(self._declared_total_pages(first_html), self._pagination_max_page(first_html))
    category_html_pages = max(self._declared_total_pages(category_html), self._pagination_max_page(category_html))
    known_pages = max(declared_max, published_pages, expected_pages, response_html_pages, category_html_pages, 1)
    pages = [category_url]
    ...
    for page_number in range(2, known_pages + 1):
        ...
        if not rendered_html:
            raise RuntimeError(f"Empty JSF pagination page {page_number} for {category_url}")
        current_product_keys = _page_product_keys(self, rendered_html, page_url)
        if not current_product_keys:
            raise RuntimeError(f"No products found on JSF pagination page {page_number} for {category_url}")
        new_product_keys = current_product_keys - seen_product_keys
        if not new_product_keys:
            raise RuntimeError(f"Repeated JSF pagination page {page_number} for {category_url}")
        seen_product_keys.update(current_product_keys)
        self._cache_category_html(page_url, rendered_html)
        pages.append(page_url)
    if known_pages > 1:
        boundary_page = known_pages + 1
        has_new_products, new_product_keys = _probe_boundary_page(self, category_url, category_id, boundary_page, seen_product_keys)
        if has_new_products:
    ...
    for offset in range(min(self.MAX_HIDDEN_PAGE_PROBES, 5)):
        ...
        page_url = self._jsf_page_url(category_url, page_number)
        _, _, rendered_html = _probe_jsf_page(self, category_url, category_id, page_number)
        if not rendered_html:
            break
        current_product_keys = _page_product_keys(self, rendered_html, page_url)
        if not current_product_keys:
...
def _get_category_pages(self: CategoryScraper, category_url: str, expected_count: int = 0) -> list[str]:
    ...
    if category_id is not None:
        self._cache_category_html(category_url, first_html)
        return _jsf_category_pages_with_probe(self, category_url, category_id, expected_count, category_html=first_html)
    direct_products = _direct_product_urls(first_html, category_url)
    if direct_products:
        direct_pages, direct_count = _facundo_direct_pages(self, category_url, first_html, expected_count)
