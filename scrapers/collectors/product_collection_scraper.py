...
class ProductCollectionScraper:
    ...
    def __init__(
        ...
        self._enrichment_metrics: dict[str, dict[str, Any]] = {}
        self._enrichment_metrics_lock = Lock()
    ...
    def _enrich_products(
        ...
        if self.detail_extractor is None or not products:
            ...
            self._store_enrichment_metrics(
                category_name=category_name,
                total_seconds=time.perf_counter() - enrichment_started,
                submit_seconds=0.0,
                wait_seconds=time.perf_counter() - enrichment_started,
                requested=0,
    ...
    def _store_enrichment_metrics(
        ...
        *,
        ...
        key = str(category_name or "").strip() or "(sin nombre)"
        with self._enrichment_metrics_lock:
            self._enrichment_metrics[key] = {
                "category": key,
                "total_seconds": float(total_seconds),
                "submit_seconds": float(submit_seconds),
                "wait_seconds": float(wait_seconds),
                "requested": int(requested),
                "skipped": int(skipped),
            }

    def get_enrichment_metrics(self, category_name: str = "") -> dict[str, Any]:
        key = str(category_name or "").strip() or "(sin nombre)"
        with self._enrichment_metrics_lock:
            return dict(self._enrichment_metrics.get(key, {}))
    ...
    def reset_detail_metrics(self) -> None:
        ...
        with self._enrichment_metrics_lock:
            self._enrichment_metrics.clear()
