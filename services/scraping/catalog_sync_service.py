...
class CatalogSyncService:
    ...
    def sync(self, products, prune_missing: bool = False, cleanup_generated: bool = True,
        ...
        for product in consolidated:
            ...
            if existing is None:
                result.created += 1
                result.changes.append({"type": "NEW", "code": product.code,
            ...
            if comparison["changed"]:
                result.updated += 1
                result.changes.append({"type": "UPDATED", "code": product.code,
                                       "name": product.name,
                                       "changes": [{"field": field,
        ...
        prune_allowed = result.coverage_complete and not result.has_errors and (
            prune_missing or result.products_expected > 0
        )
        if prune_allowed:
