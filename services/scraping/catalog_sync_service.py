...
class CatalogSyncService:
    ...
    def sync(self, products, prune_missing: bool = False, cleanup_generated: bool = True,
        ...
        missing_code_products = [p for p in raw_products if not str(getattr(p, "code", "")).strip()]
        for product in missing_code_products:
            result.missing_code += 1
            result.changes.append({"type": "MISSING_CODE", "code": "",
                                   "name": str(getattr(product, "name", "")).strip(),
                                   "changes": [{"field": "code", "label": "Código no encontrado",
                                                "old": "Sin código", "new": "Ignorado"}]})
        prepared = [p for p in raw_products if str(getattr(p, "code", "")).strip()]
        consolidated = self.consolidate_products(prepared)
        result.products_expected = max(int(expected_products or 0), 0)
        result.expected_category_occurrences = max(int(expected_category_occurrences or 0), 0)
        result.products_found = len(raw_products)
        result.products_unique = len(consolidated)
        result.processed = result.products_found
        result.duplicate_occurrences = max(result.products_found - result.products_unique, 0)
        result.products_multiple_categories = self._count_multi_category_products(prepared)
        scraped_codes = {str(product.code).strip().casefold() for product in consolidated}
        for product in consolidated:
            existing = self.repository.get(product.code)
            if existing is None:
                result.created += 1
                result.changes.append({"type": "NEW", "code": product.code, "name": product.name, "changes": []})
            ...
            if comparison["changed"]:
                result.updated += 1
                result.changes.append({"type": "UPDATED", "code": product.code, "name": product.name,
                                       "changes": [{"field": field, "label": self.FIELD_LABELS.get(field, field),
                ...
                result.unchanged += 1
        prune_allowed = result.coverage_complete and not result.has_errors and (prune_missing or result.products_expected > 0)
        if prune_allowed:
    ...
    def sync_full_catalog(self, products, prune_missing: bool = True, expected_products: int | None = 0,
                          expected_category_occurrences: int = 0):
        return self.sync(products, prune_missing=prune_missing, expected_products=expected_products,
    ...
    def _remove_missing_products(self, scraped_codes: set[str], result: SyncResult) -> None:
        for existing in self.repository.get_all():
            ...
            if not code or code.casefold() in scraped_codes:
                continue
            result.deleted += 1
            result.changes.append({"type": "DELETED", "code": code, "name": getattr(existing, "name", ""),
                                   "changes": [{"field": "code", "label": "Código ausente en origen",
    ...
    def consolidate_products(cls, products):
        ...
        for product in products:
            ...
            if existing is None:
                ...
                continue
            existing.category = cls._merge_categories(existing.category, product.category)
            colors = list(getattr(existing, "colors", [])) + list(getattr(product, "colors", []))
            existing.colors = list(dict.fromkeys(str(c).strip() for c in colors if str(c).strip()))
            ...
            for color, stock in getattr(product, "color_stock", {}).items():
                ...
                try:
                    ...
                    continue
                color_stock[normalized_color] = max(color_stock.get(normalized_color, 0), normalized_stock)
            existing.color_stock = color_stock
            if not getattr(existing, "description", "") and getattr(product, "description", ""):
                existing.description = product.description
            if not getattr(existing, "image_url", "") and getattr(product, "image_url", ""):
