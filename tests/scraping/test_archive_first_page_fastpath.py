...
def test_archive_first_page_is_recoverable_after_boundary_http_error():
    category_url = (
        "https://stock.importacionesfacundo.com/"
        "categoria-producto/catalogo-recovery/"
    )
    ...
    responses = {
        category_url: (
            '<html><body class="product_cat-999">'
            + products(1, 25)
            + "</body></html>"
        ),
        "ajax:2": jsf_response(26, 25, 50, 2),
        "ajax:3": requests.HTTPError(response=response),
    }
    browser = FakeBrowser(responses)
    scraper = ResilientCategoryScraper(
        browser=browser,
        category_extractor=object(),
    )
