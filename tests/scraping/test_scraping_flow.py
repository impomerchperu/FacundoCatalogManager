...
def test_scraping_complete_flow():
    ...
    service = ScrapedProductService(repository, scraper, mapper)

    product = service.scrape_and_save("https://example.com/producto")

    assert product.name == "Producto Demo"

    saved = repository.get_by_url("https://example.com/producto")
